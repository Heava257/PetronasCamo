...
async function check() {
  try {
    ...
    const purchases = results.filter(r => r.transaction_type === 'purchase');
    console.log("ALL PAYMENTS:", JSON.stringify(payments, null, 2));

    // Check for ID collision
    const paymentIds = new Set(payments.map(p => p.id));
    const collisions = purchases.filter(p => paymentIds.has(p.id));
    if (collisions.length > 0) {
      console.log("COLLISIONS DETECTED (Overlap IDs):", collisions.map(c => c.id));
    } else {
      console.log("No ID collisions detected between payments and purchases.");
    }
    process.exit(0);
  } catch (error) {

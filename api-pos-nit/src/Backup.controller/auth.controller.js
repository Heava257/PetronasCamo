...
// // ✅ Fixed logLoginActivity function
...
//       loginData.location_info,
//       loginData.login_time,
//       loginData.status
//     ]);
//   } catch (error) {
...
//         WHERE session_token = :session_token AND user_id = :user_id
//       `, {
...
exports.login = async (req, res) => {
  try {
    ...
    // ⚠️ USERNAME DOESN'T EXIST - Return 401 status
    if (data.length == 0) {
      return res.status(401).json({
        error: {
          username: "Username doesn't exist!",
    ...
    // ⚠️ WRONG PASSWORD - Return 401 status
    if (!isCorrectPass) {
    ...
    // ✅ LOGIN SUCCESSFUL - Continue with authentication
    delete data[0].password;
    let obj = {
      profile: data[0],
      permission: await getPermissionByUser(data[0].id),
...
ប្រសិនបើមិនមែនជាអ្នក សូមប្តូរពាក្យសម្ងាត់របស់អ្នកភ្លាមៗ!
    ...
    // Log successful login activity to database
    try {
      await exports.logLoginActivity({
        user_id: data[0].id,
        username: data[0].username,

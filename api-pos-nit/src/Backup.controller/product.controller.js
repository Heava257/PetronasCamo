...
exports.create = async (req, res) => {
  try {
    const {
      ...
      customer_id, order_date  // ✅ បន្ថែម order_date
    } = req.body;
    ...
    if (existingProducts.length > 0) {
      ...
      await db.query(updateSql, {
        ...
        order_date: order_date || new Date(),  // ✅ Auto current date if not provided
      ...
      // ✅ Insert new product WITH barcode and order_date
      const insertSql = `
        INSERT INTO product 
...
📅 <b>Order Date:</b> ${orderDateFormatted}
...
exports.getProductDetail = async (req, res) => {
  try {
    ...
    // ✅ Payment query - ប្រើ date filtering ដូចគ្នា
    ...
    if (order_date) {
      ...
      paymentWhere += ` AND DATE(pay.payment_date) >= DATE(?) AND DATE(pay.payment_date) <= DATE(?)`;
...
exports.sendBatchTelegramNotification = async (products) => {
  ...
  if (cardNumbers.length > 0) {
    message += `• <b>Invoice Number:</b> ${cardNumbers[0]}\n`;
  }
  message += `━━━━━━━━━━━━━━━━━`;
  let grandTotal = 0;

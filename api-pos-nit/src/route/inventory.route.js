...
module.exports = (app) => {
    ...
    app.get("/api/inventory/transactions", validate_token(), getTransactionList);

    // GET /api/inventory/statistics - Get inventory statistics
    app.get("/api/inventory/statistics", validate_token(), getStatistics);

    // ✅ Get stock breakdown by category (Gasoline, Diesel, etc.)
    app.get("/api/inventory/category-statistics", validate_token(), getCategoryStatistics);

    // GET /api/inventory/recent - Get recent transactions
    app.get("/api/inventory/recent", validate_token(), getRecentTransactions);
    ...
    app.get("/api/inventory/latest-selling-price", validate_token(), getLatestSellingPrice);

    // Get customer product price
    app.get("/api/inventory/customer-product-price", validate_token(), getCustomerProductPrice);
};